...
Usage:
    python track_activations_gradients_first_epoch.py outputs/subfolder/sweep_name/
"""
...
def track_first_epoch(output_dir: str):
    ...
    tracking_data = {
        'epoch': 0,
        'steps': [],
        'activations': {point: {stat: [] for stat in ['mean', 'std', 'norm', 'max', 'sparsity']} 
    ...
    def make_activation_hook(point_name):
        def hook(module, input, output):
            activation_storage[point_name] = output.detach()
        return hook
    
    # Register hooks on activation points (we'll need to modify forward or use a wrapper)
    # For now, we'll modify the forward pass to capture activations
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python track_activations_gradients_first_epoch.py <output_dir>")
        print("Example: python track_activations_gradients_first_epoch.py outputs/gradients_across_training/three_layer_skip_with_50_100_150/sweep_.../")
        sys.exit(1)
