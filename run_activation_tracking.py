...
Usage:
    python run_activation_tracking.py --subfolder three_layer_skip_with_50_100_150 --max-workers 6
"""
...
def run_single_tracking(output_dir):
    ...
    try:
        ...
        if not config_path.exists():
            return {'output_dir': str(output_dir), 'status': 'no_config', 'error': 'config.json not found'}
            ...
            if len(parts) == 2:
                ...
                if 'outputs' in path_parts:
                    ...
                    if idx + 2 < len(path_parts):
                        ...
                        save_dir = Path('outputs') / 'activation_gradient_analysis_first_epoch' / subfolder / sweep_name
                    else:
                        save_dir = None
                else:
                    save_dir = None
        ...
        if save_dir and save_dir.exists():
            ...
            if output_file.exists():
                return {'output_dir': str(output_dir), 'status': 'skipped', 'reason': 'already exists'}
        
        print(f"Starting {output_dir.name}")
        
        result = subprocess.run(
            ['python3', 'track_activations_gradients_first_epoch.py', str(output_dir)],
            capture_output=True,
            text=True,
        ...
        if result.returncode == 0:
            ...
            return {'output_dir': str(output_dir), 'status': 'failed', 'error': error_msg[:500], 'exit_code': result.returncode}
...
def main():
    ...
    if args.max_workers is None:
        try:
            ...
            if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
                ...
                print(f"Detected MPS device - limiting to {args.max_workers} workers for stability")
    ...
    print(f"Results will be saved to: outputs/activation_gradient_analysis_first_epoch/{args.subfolder}/")
    results = []
    ...
    print(f"  Results saved to: outputs/activation_gradient_analysis_first_epoch/{args.subfolder}/")
    
    # Save results summary
    summary_dir = Path('outputs') / 'activation_gradient_analysis_first_epoch' / args.subfolder
