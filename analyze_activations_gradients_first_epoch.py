...
Usage:
    python analyze_activations_gradients_first_epoch.py --subfolder three_layer_skip_with_50_100_150
"""
...
def compute_loss_accuracy_metrics(tracking_data):
    ...
    if 'accuracy' in tracking_data and tracking_data['accuracy']:
        ...
        metrics['accuracy_change'] = tracking_data['accuracy'][-1] - tracking_data['accuracy'][0]
...
def main():
    ...
    tracking_dirs = sorted([d for d in base_dir.iterdir() 
    ...
    print(f"\nColumn categories:")
    print(f"  Hyperparameters: {len([c for c in df.columns if c in hyperparams])}")
    print(f"  Activation metrics: {len([c for c in df.columns if any(ap in c for ap in ['x1', 'x2', 'x3', 'x4'])])}")
    print(f"  Gradient metrics: {len([c for c in df.columns if '_grad_' in c])}")
    print(f"  Loss/Accuracy metrics: {len([c for c in df.columns if c.startswith(('loss_', 'accuracy_'))])}")
