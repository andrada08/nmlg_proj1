...
subfolder_name = "four_layer_integrating_with_30"  # Default subfolder name
...
all_layer_sizes = list(product(layer_size_options, repeat=4))

# Filter to only include combinations where at least one layer is 30 (new combinations)
layer_sizes = [(l1, l2, l3, l4) for (l1, l2, l3, l4) in all_layer_sizes if 30 in (l1, l2, l3, l4)]
...
for (l1, l2, l3, l4), (pattern_name, lr_dict) in product(layer_sizes, lr_patterns.items()):
    ...
    cfg = template | {"hidden_sizes": [l1, l2, l3, l4], "layer_lns": lr_dict}
