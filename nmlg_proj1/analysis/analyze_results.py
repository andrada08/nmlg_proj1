...
def main():
    ...
    if not input_folder and not output_subfolder:
        print(
            "Error: Provide either --input-folder outputs/{subfolder} or --output-subfolder {subfolder}."
        )
        return
    ...
    for output_dir in sorted(output_dirs):
        ...
        try:
            ...
            row = {"run_name": os.path.basename(output_dir), **hyperparams, **metrics}
    ...
    for pattern, count in pattern_counts.items():
        ...
        if count > 0:
            ...
            alignment_col = (
                f'{pattern.replace("_pattern", "")}_pattern_boost_pattern_aligned'
            )
            if alignment_col in df.columns:
                ...
                print(
                    f"    └─ Aligned: {aligned}/{count} ({aligned_pct:.1f}%), Not aligned: {not_aligned}/{count} ({100-aligned_pct:.1f}%)"
                )
    ...
    for _, row in top_runs.iterrows():
        print(
            f"  {row['run_name']}: {row['pattern_count']} patterns, {row['final_test_accuracy']:.2f}% accuracy"
        )

    print("\nTo create visualizations, run:")
    print(
        f"  python3 -m nmlg_proj1.plotting.plot_results --pattern-analysis --input-folder {analysis_folder}"
    )
